...
namespace CoffeShop.ViewModel
{
    ...
    {
        public LoginWindowViewModel lgVM;
        #region command
        ...
        {
            lgVM = new LoginWindowViewModel();
            CloseWindowCommand = new RelayCommand<UserControl>((p) => { return p == null ? false : true; }, (p) =>
            {
                FrameworkElement window = GetWindowParent(p);
                var w = window as Window;
                var mw = window as MainWindow;
                var lg = window as LoginWindow;
                if (w == mw && w!=lg)
                {
                    var userLogin = dataProvider.Ins.DB.Accounts.Where(x => x.Online == true).ToList();
